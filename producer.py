...
# List of Phone Numbers and Locations
phone_numbers = [
    6285112345678,
    6285224681359,
    6285335792460,
...
locations = ["Blok M", "ASEAN", "Blok A", "Fatmawati", "Lebak Bulus", "Cipete", "Haji Nawi", "Senayan"]
...
TRANSACTION_SCHEMA = """
    {
      ...
      "title": "Transaction",
      "description": "Telco Billing transaction",
      "type": "object",
      "properties": {
        "amount": {
            "connect.index": 3,
...
class Transaction(object):
    def __init__(self, transaction_id, transaction_timestamp, phone_number, amount, location):
...
KAFKA_TOPIC = 'transactions_telco'  # Kafka topic for sending transaction data
...
def generate_fraudulent_transactions(phone_number, start_time, num_transactions=3):
    transactions = []
...
def generate_transactions():
    ...
    fraudulent_trans = None

    while True:
        # Randomly choose a phone number for normal transactions
        phone_nums = random.choice(phone_numbers)
        ...
        if random_num < 0.1:  # 10% chance of initiating fraudulent transactions
                fraudulent_trans = random.choice(phone_numbers)
                last_fraud_time = datetime.now()

                # Generate fraudulent transactions
                fraudulent_transactions = generate_fraudulent_transactions(fraudulent_trans, last_fraud_time)
